...
simple_ilist<DPValue>::iterator DPMarker::cloneDbgInfoFrom(DPMarker *From, bool InsertAtHead) {
  DPValue *First = nullptr;
  if (InsertAtHead) {
    for (DPValue &DPV : reverse(From->StoredDPValues)) {
    ...
    for (DPValue &DPV : From->StoredDPValues) {
      DPValue *New = DPV.clone();
      New->setMarker(this);
