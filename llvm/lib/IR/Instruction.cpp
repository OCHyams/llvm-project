...
void Instruction::moveBefore1(BasicBlock &BB,
  ...
  assert(!getParent()->IsInhaled || DbgMarker);
...
BasicBlock::DIIterator Instruction::cloneDebugInfoFrom(const Instruction *From, bool InsertAtHead) {
  if (!DbgMarker)
  ...
  return DbgMarker->cloneDbgInfoFrom(From->DbgMarker, InsertAtHead);
}
