...
MCObjectStreamer::emitDwarfLoclistElem(int8_t OffsetPair, const MCSymbol *Base,
  ...
  bool IsLinkerRelaxable = Base->getFragment()->LinkerRelaxable ||
                           Begin->getFragment()->LinkerRelaxable ||
                           End->getFragment()->LinkerRelaxable;
  if (IsLinkerRelaxable || Base->getFragment() == Begin->getFragment() ||
      Base->getFragment() == End->getFragment())
    return MCStreamer::emitDwarfLoclistElem(OffsetPair, Base, Begin, End,
                                            EnumEle);

  MCDwarfLocListOffsetPairFragment *ptr =
