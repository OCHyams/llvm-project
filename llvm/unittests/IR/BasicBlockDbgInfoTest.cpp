...
TEST(BasicBlockDbgInfoTest, MarkerOperations) {
  ...
  DPValue *DPV1 = cast<DPValue>(Marker1->StoredDbgRecords.begin());
  DPValue *DPV2 = cast<DPValue>(Marker2->StoredDbgRecords.begin());
...
TEST(BasicBlockDbgInfoTest, InstrDbgAccess) {
  ...
  DbgRecord *DPE1 = &*CInst->DbgMarker->StoredDbgRecords.begin();
  ASSERT_TRUE(DPE1);
  ...
  DbgRecord *DPE2 = &*BInst->DbgMarker->StoredDbgRecords.begin();
  EXPECT_EQ(std::distance(Range1.begin(), Range1.end()), 1u);
...
protected:
  ...
  bool InstContainsDPValue(Instruction *I, DPValue *DPV) {
    for (DPValue &D : filterValues(I->getDbgRecordRange())) {
      if (&D == DPV) {
...
TEST(BasicBlockDbgInfoTest, DbgSpliceToEmpty1) {
  ...
  for (DPValue &DPV : filterValues(BInst->getDbgRecordRange()))
