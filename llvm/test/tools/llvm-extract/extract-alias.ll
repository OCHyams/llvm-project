; RUN: llvm-extract -coffee-verify-each=false -func foo -S < %s | FileCheck %s
; RUN: llvm-extract -coffee-verify-each=false -delete -func foo -S < %s | FileCheck --check-prefix=DELETE %s
; RUN: llvm-extract -coffee-verify-each=false -alias zeda0 -S < %s | FileCheck --check-prefix=ALIAS %s
; RUN: llvm-extract -coffee-verify-each=false -ralias '.*bar' -S < %s | FileCheck --check-prefix=ALIASRE %s
