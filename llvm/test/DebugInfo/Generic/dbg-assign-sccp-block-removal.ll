...
;; SCCP deletes the contents of unreachable blocks. If the only use of a
;; DIAssignID comes from an unreachable block the definition must be
;; removed. I.e. The DIAssignID must be removed from the assigning instruction.
...
;; Double check that the dbg.assign has been deleted by SCCP.
; CHECK: if.then:
