...
; CHECK: distinct !DISubprogram(name: "foo", {{.*}}nextAtomGroup: 2)
; CHECK-DAG: ![[RET1]] = !DILocation(line: 1, {{.*}}, atomGroup: 1, atomRank: 1
; CHECK: distinct !DISubprogram(name: "bar", {{.*}}nextAtomGroup: 5)
...
; CHECK: distinct !DISubprogram(name: "boom", {{.*}}nextAtomGroup: 7)
