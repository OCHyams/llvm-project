...
;; -- sunk dbg.assigns
; CHECK: call void @llvm.dbg.assign(metadata float undef, metadata ![[var:[0-9]+]], metadata !DIExpression(), metadata ![[id:[0-9]+]], metadata float* %h), !dbg
; CHECK-NEXT: call void @llvm.dbg.assign(metadata float undef, metadata ![[var]], metadata !DIExpression(), metadata ![[id]], metadata float* %h), !dbg
; CHECK-NEXT: %storemerge.in = getelementptr
; CHECK-NEXT: %storemerge = load float
; CHECK-NEXT: store float %storemerge, float* %h, align 4{{.+}}!DIAssignID ![[id]]
; CHECK: ret void
...
define dso_local void @_ZN1e1iEv(%class.e* %this) local_unnamed_addr #0 align 2 !dbg !11 {
entry:
  %h = alloca float, align 4, !DIAssignID !32
  call void @llvm.dbg.assign(metadata i1 undef, metadata !31, metadata !DIExpression(), metadata !32, metadata float* %h), !dbg !33
  %0 = bitcast float* %h to i8*, !dbg !34
  call void @llvm.lifetime.start.p0i8(i64 4, i8* nonnull %0) #4, !dbg !34
  call void @_Zml1aRf(float* nonnull align 4 dereferenceable(4) %h), !dbg !35
  %1 = load float, float* %h, align 4, !dbg !36, !tbaa !38
  %tobool = fcmp une float %1, 0.000000e+00, !dbg !36
  br i1 %tobool, label %if.then, label %if.else, !dbg !42

if.then:                                          ; preds = %entry
  call void @llvm.dbg.assign(metadata float undef, metadata !31, metadata !DIExpression(), metadata !43, metadata float* %h), !dbg !33
...
if.end:                                           ; preds = %if.else, %if.then
  %storemerge.in = getelementptr inbounds %class.e, %class.e* %this, i64 0, i32 1, !dbg !45
  %storemerge = load float, float* %storemerge.in, align 4, !dbg !45, !tbaa !46
  store float %storemerge, float* %h, align 4, !dbg !45, !tbaa !38, !DIAssignID !43
...
declare !dbg !49 dso_local void @_Zml1aRf(float* nonnull align 4 dereferenceable(4)) local_unnamed_addr #2
