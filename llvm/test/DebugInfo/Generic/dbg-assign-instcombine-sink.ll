...
; CHECK: f.exit:
; CHECK-NEXT: %b = getelementptr inbounds %struct.a, %struct.a* %i, i64 0, i32 0, !dbg
; CHECK-NEXT: store i8* null, i8** %b, align 8,{{.+}}, !DIAssignID ![[ID:[0-9]+]]
; CHECK-NEXT: call void @llvm.dbg.assign({{.+}}, {{.+}}, {{.+}}, metadata ![[ID]], metadata i8** %b), !dbg
...
if.then:                                          ; preds = %entry
  %0 = load i32, i32* @d, align 4, !dbg !17, !tbaa !18
  ret i32 %0, !dbg !22

if.end:                                           ; preds = %entry
  %call1 = tail call i8* (...) @c() #6, !dbg !23
  unreachable, !dbg !23
}
...
declare !dbg !27 dso_local i8* @c(...) local_unnamed_addr #2

; Function Attrs: nounwind uwtable
define dso_local void @h() local_unnamed_addr #0 !dbg !35 {
entry:
  %i = alloca %struct.a, align 8, !DIAssignID !40
  call void @llvm.dbg.assign(metadata i1 undef, metadata !39, metadata !DIExpression(), metadata !40, metadata %struct.a* %i), !dbg !41
  %0 = bitcast %struct.a* %i to i8*, !dbg !42
  call void @llvm.lifetime.start.p0i8(i64 8, i8* nonnull %0) #5, !dbg !42
  %b = getelementptr inbounds %struct.a, %struct.a* %i, i64 0, i32 0, !dbg !43
  call void @llvm.dbg.assign(metadata i8* null, metadata !39, metadata !DIExpression(), metadata !44, metadata i8** %b), !dbg !41
...
if.end.i:                                         ; preds = %entry
  %call1.i = tail call i8* (...) @c() #6, !dbg !48
  unreachable, !dbg !48

f.exit:                                           ; preds = %entry
  store i8* null, i8** %b, align 8, !dbg !49, !tbaa !50, !DIAssignID !53
  call void @llvm.dbg.assign(metadata i8* null, metadata !39, metadata !DIExpression(), metadata !53, metadata i8** %b), !dbg !41
  call void (%struct.a*, ...) bitcast (void (...)* @g to void (%struct.a*, ...)*)(%struct.a* nonnull %i) #5, !dbg !54
  call void @llvm.lifetime.end.p0i8(i64 8, i8* nonnull %0) #5, !dbg !55
  ret void, !dbg !55
}
