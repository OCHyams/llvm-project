...
public:
  ...
  /// Convert variable location debugging information stored in dbg.*
  /// intrinsics into DPMarker / DbgRecords. Deletes all dbg intrinsics in the
  /// process and sets IsNewDbgInfoFormat = true. Only takes effect if the
  /// UseNewDbgInfoFormat LLVM command line option is given.
  void convertToNewDbgInfoFormat();

  /// Convert variable location debugging information stored in DPMarkers and
  /// DbgRecords into the dbg.* intrinsic representation. Sets
  /// IsNewDbgInfoFormat = false.
  void convertFromNewDbgInfoFormat();
  ...
  /// Validate any DPMarkers / DbgRecords attached to instructions in this block,
  /// and block-level stored data too (TrailingDbgRecords).
  ...
  /// instruction of this block. These are equivalent to dbg.* intrinsics
  ...
  /// move such DbgRecord back to the right place (ahead of the terminator).
  ...
  /// does not have an equivalent with DbgRecord.
