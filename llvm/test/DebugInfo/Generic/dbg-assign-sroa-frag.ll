...
;;
;; Check that when the memcpy to fragment(256, 128) is split into two
;; 2xfloat stores, the dbg.assign is split into two with
;; fragment(256, 64) & fragment(320, 64).

; CHECK: %call = call
; CHECK-NEXT: %0 = extractvalue { <2 x float>, <2 x float> } %call, 0
; CHECK-NEXT: %1 = extractvalue { <2 x float>, <2 x float> } %call, 1
; CHECK-NEXT: call void @llvm.dbg.assign(metadata <2 x float> %0, metadata ![[var:[0-9]+]], metadata !DIExpression(DW_OP_LLVM_fragment, 256, 64),{{.+}},{{.+}}undef), !dbg
; CHECK-NEXT: call void @llvm.dbg.assign(metadata <2 x float> %1, metadata ![[var]], metadata !DIExpression(DW_OP_LLVM_fragment, 320, 64),{{.+}},{{.+}}undef), !dbg
...
entry:
  ...
  call void @llvm.dbg.assign(metadata i1 undef, metadata !11, metadata !DIExpression(), metadata !22, metadata [3 x %class.c]* %a), !dbg !23
  %ref.tmp = alloca %class.c, align 4
  %0 = bitcast [3 x %class.c]* %a to i8*, !dbg !24
  call void @llvm.lifetime.start.p0i8(i64 48, i8* %0) #4, !dbg !24
  %1 = bitcast %class.c* %ref.tmp to i8*, !dbg !25
  call void @llvm.lifetime.start.p0i8(i64 16, i8* %1) #4, !dbg !25
  %call = call { <2 x float>, <2 x float> } @_Z3fn1v(), !dbg !25
  %coerce.dive = getelementptr inbounds %class.c, %class.c* %ref.tmp, i32 0, i32 0, !dbg !25
  %2 = bitcast [4 x float]* %coerce.dive to { <2 x float>, <2 x float> }*, !dbg !25
  %3 = getelementptr inbounds { <2 x float>, <2 x float> }, { <2 x float>, <2 x float> }* %2, i32 0, i32 0, !dbg !25
  %4 = extractvalue { <2 x float>, <2 x float> } %call, 0, !dbg !25
  store <2 x float> %4, <2 x float>* %3, align 4, !dbg !25
  %5 = getelementptr inbounds { <2 x float>, <2 x float> }, { <2 x float>, <2 x float> }* %2, i32 0, i32 1, !dbg !25
  %6 = extractvalue { <2 x float>, <2 x float> } %call, 1, !dbg !25
  store <2 x float> %6, <2 x float>* %5, align 4, !dbg !25
  %arrayidx = getelementptr inbounds [3 x %class.c], [3 x %class.c]* %a, i64 0, i64 2, !dbg !26
  %7 = bitcast %class.c* %arrayidx to i8*, !dbg !27
  %8 = bitcast %class.c* %ref.tmp to i8*, !dbg !27
  call void @llvm.memcpy.p0i8.p0i8.i64(i8* align 16 %7, i8* align 4 %8, i64 16, i1 false), !dbg !27, !tbaa.struct !28, !DIAssignID !32
  call void @llvm.dbg.assign(metadata i1 undef, metadata !11, metadata !DIExpression(DW_OP_LLVM_fragment, 256, 128), metadata !32, metadata i8* %7), !dbg !23
  %9 = bitcast %class.c* %ref.tmp to i8*, !dbg !26
  call void @llvm.lifetime.end.p0i8(i64 16, i8* %9) #4, !dbg !26
  %10 = bitcast [3 x %class.c]* %a to i8*, !dbg !33
  call void @llvm.lifetime.end.p0i8(i64 48, i8* %10) #4, !dbg !33
  ret void, !dbg !33
}
