...
// CHECK: DISubprogram(name: "a"{{.*}}, nextAtomGroup: 5)
...
// CHECK: DISubprogram(name: "b"{{.*}}, nextAtomGroup: 2)
// CHECK: [[B_G1R1]] = !DILocation({{.*}}, atomGroup: 1, atomRank: 1)
// CHECK: DISubprogram(name: "c"{{.*}}, nextAtomGroup: 2)
...
// CHECK: DISubprogram(name: "e"{{.*}}, nextAtomGroup: 2)
// CHECK: [[E_G1R1]] = !DILocation({{.*}}, atomGroup: 1, atomRank: 1)
// CHECK-CXX: DISubprogram(name: "f"{{.*}}, nextAtomGroup: 5)
