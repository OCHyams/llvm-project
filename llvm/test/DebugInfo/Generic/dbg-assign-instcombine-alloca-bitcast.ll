...
;;
;; Reduced from the (SN) dexter test nostdlib/struct_small_members.
;; $ cat reduce.cpp
;; struct c {
;;   c(int);
;;   int a, b;
...
;;   return e;
;; }
;; $ -O2 -c -g reduce.cpp -o test.ll -fno-inline  -Xclang -debug-coffee-chat -Xclang -disable-llvm-passes -emit-llvm -S

; CHECK: entry:
; CHECK-NEXT: %retval = alloca i64, align 8, !DIAssignID ![[ID:[0-9]+]]
; CHECK-NEXT: %tmpcast = bitcast i64* %retval to %struct.c*
; CHECK-NEXT: call void @llvm.dbg.assign(metadata i1 undef, metadata ![[e:[0-9]+]], metadata !DIExpression(), metadata ![[ID]], metadata i64* %retval), !dbg
; CHECK: ![[e]] = !DILocalVariable(name: "e",

source_filename = "reduce.cpp"
...
entry:
  ...
  call void @llvm.dbg.assign(metadata i1 undef, metadata !20, metadata !DIExpression(), metadata !21, metadata %struct.c* %retval), !dbg !22
